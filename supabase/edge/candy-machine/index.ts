/**
 * supabase/functions/candy-machine/index.ts
 *
 * Supabase Edge Function for Diggle NFT Candy Machine.
 * Single "fat function" handling two routes:
 *
 *   GET  /candy-machine/mint-info      — Candy machine state (items left, price, etc.)
 *   POST /candy-machine/build-mint-tx  — Build unsigned mintV2 transaction
 *
 * The Flutter app calls build-mint-tx, signs via MWA, and sends to the network.
 *
 * Required secrets (set via `supabase secrets set --env-file .env.local`):
 *   SOLANA_RPC_URL  — Your Solana RPC endpoint
 *
 * URL: https://vdcpbqsnkivokroqxelq.supabase.co/functions/v1/candy-machine
 * Deploy:
 *   supabase functions deploy candy-machine --no-verify-jwt
 */
...
Deno.serve(async (req: Request) => {
  ...
  // Edge function is mounted at /candy-machine, so strip that prefix
  const path = url.pathname.replace(/^\/candy-machine\/?/, '/');
